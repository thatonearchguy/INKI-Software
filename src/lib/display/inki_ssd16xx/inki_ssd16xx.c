 ...
 *
 * DISCLAIMER: FOR NEA PURPOSES, THIS IS NOT MY CODE. THIS IS A PREBUILT DRIVER THAT I HAVE SLIGHTLY MODIFIED TO 
 * ADD 4 GRAYSCALE AND PARTIAL REFRESH SUPPORT USING MY 4g-epd1in54 LIBRARY. THIS CODE BELONGS TO PHYTEC MESSTECHNIC GMBH
 * AND THEY RESERVE ALL RIGHTS UNDER THE LICENSE DENOTED BELOW.
 *
 * SPDX-License-Identifier: Apache-2.0
 */

#define DT_DRV_COMPAT inki_ssd16xxfb

#define LOG_LEVEL CONFIG_DISPLAY_LOG_LEVEL
#include <logging/log.h>
LOG_MODULE_REGISTER(inki_ssd16xx);
			 ...
			 const void *buf)
{
	...
	err = ssd16xx_write_cmd(driver, SSD16XX_CMD_WRITE_RAM_1, (uint8_t *)buf,
				buf_len);
	if (err < 0) {
		return err;\
	}
...
static int ssd16xx_controller_init(const struct device *dev)
{
	...
	err = ssd16xx_clear_cntlr_mem(dev, SSD16XX_CMD_WRITE_RAM_1, true);
	if (err < 0) {
		return err;
	}
	...
	err = ssd16xx_clear_cntlr_mem(dev, SSD16XX_CMD_WRITE_RAM_2,
					     false);
	if (err < 0) {
		return err;
	}
	...
	return ssd16xx_clear_cntlr_mem(dev, SSD16XX_CMD_WRITE_RAM_1, true);
}
