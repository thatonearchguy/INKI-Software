...
#define mod_name nrf53_lp_uarte_drv_dev
...
#define DPPI_CHANNELS_REQUIRED 4 
 ...
 * */
#define GET_DPPI_FROM_VEC(vector, index, name) \
uint8_t* CONCAT(ppi_ptr_, index) = (uint8_t*)vector_get(&vector, index); \
uint8_t name = *CONCAT(ppi_ptr_, index); \
free(CONCAT(ppi_ptr_, index)); \

#define FREE_REALLOC_DPPI(vector, index, name) \
GET_DPPI_FROM_VEC(vector, index, name); \
err_code = nrfx_dppi_channel_disable(name); \
LP_DRV_ERR_CHECK(nrfx_err_to_zephyr_err(err_code), "Error disabling DPPI channel"); \


nrf_uarte_config_t nrf_config;
NRF_UARTE_Type nrf_uarte_reg;
NRF_DPPIC_Type nrf_dppi_reg;
...
static int nrf_lp_uarte_gpiote_rx_init(const struct lp_uarte_dev* dev, bool ft_en)
{
    if(curr_state != ((ft_en) ? NRF_UARTE_STATE_RX_FT : NRF_UARTE_STATE_RX_NO_FT))
    {
        ...
        GET_DPPI_FROM_VEC(ppi_channels, 0, vec_ppi_0);
        nrf_uarte_subscribe_set(&nrf_uarte_reg, NRF_UARTE_TASK_STARTRX, vec_ppi_0); //nrf53 specific

        err_code = nrfx_dppi_channel_assign(vec_ppi_0, (uint32_t)&NRF_GPIOTE->EVENTS_IN[0], nrf_uarte_task_address_get(&nrf_uarte_reg, NRF_UARTE_TASK_STARTRX)); //assigning S0 lo-hi to radio RX begin. 
        LP_DRV_ERR_CHECK(nrfx_err_to_zephyr_err(err_code), "Error assigning S0 rising edge event to STARTRX task");

        GET_DPPI_FROM_VEC(ppi_channels, 1, vec_ppi_1);
        //nrf_uarte_subscribe_set(&nrf_uarte_reg, NRF_UARTE_TASK_STOPRX, *vec_ppi);
        err_code = nrfx_dppi_channel_assign(vec_ppi_1, (uint32_t)&NRF_GPIOTE->EVENTS_IN[1], nrf_uarte_task_address_get(&nrf_uarte_reg, NRF_UARTE_TASK_STOPRX));
        LP_DRV_ERR_CHECK(nrfx_err_to_zephyr_err(err_code), "Error assigning S0 falling edge event to STOPRX task");

        if(ft_en)
        {
            GET_DPPI_FROM_VEC(ppi_channels, 2, vec_ppi_2);
            //nrf_uarte_publish_set(&nrf_uarte_reg, NRF_UARTE_EVENT_RXSTARTED, *vec_ppi);
            err_code = nrfx_dppi_channel_assign(vec_ppi_2, nrf_uarte_event_address_get(&nrf_uarte_reg, NRF_UARTE_EVENT_ENDRX), nrf_qspi_task_address_get(&nrf_qspi_reg, NRF_QSPI_TASK_WRITESTART));
            LP_DRV_ERR_CHECK(nrfx_err_to_zephyr_err(err_code), "Error assigning ENDRX event to QSPI write task configured by ISR.");

            GET_DPPI_FROM_VEC(ppi_channels, 3, vec_ppi_3);
            //nrf_uarte_publish_set(&nrf_uarte_reg, NRF_UARTE_EVENT_RXSTARTED, *vec_ppi);
            err_code = nrfx_dppi_channel_assign(vec_ppi_3, nrf_qspi_event_address_get(&nrf_qspi_reg, NRF_QSPI_EVENT_READY), (uint32_t)&NRF_GPIOTE->TASKS_CLR[2]);
        ...
        else
        {
            GET_DPPI_FROM_VEC(ppi_channels, 2, vec_ppi_2);
            //nrf_uarte_publish_set(&nrf_uarte_reg, NRF_UARTE_EVENT_RXSTARTED, *vec_ppi);
            err_code = nrfx_dppi_channel_assign(vec_ppi_2, nrf_uarte_event_address_get(&nrf_uarte_reg, NRF_UARTE_EVENT_RXSTARTED), (uint32_t)&NRF_GPIOTE->TASKS_SET[2]);
            ...
            GET_DPPI_FROM_VEC(ppi_channels, 3, vec_ppi_3);
            //nrf_uarte_publish_set(&nrf_uarte_reg, NRF_UARTE_EVENT_RXSTARTED, *vec_ppi);
            err_code = nrfx_dppi_channel_assign(vec_ppi_3, nrf_uarte_event_address_get(&nrf_uarte_reg, NRF_UARTE_EVENT_ENDRX), (uint32_t)&NRF_GPIOTE->TASKS_CLR[2]);
            LP_DRV_ERR_CHECK(nrfx_err_to_zephyr_err(err_code), "Error assigning S1 hi to lo to ENDRX");
        }

        IRQ_DIRECT_CONNECT(SERIAL0_IRQn, 4, uarte_rxtx_handler, 0);
...
static int nrf_lp_uarte_gpiote_tx_init(const struct lp_uarte_dev* dev, bool ft_en)
{
    ...
    if(curr_state != ((ft_en) ? NRF_UARTE_STATE_TX_FT : NRF_UARTE_STATE_TX_NO_FT))
    {
        ...
        GET_DPPI_FROM_VEC(ppi_channels, 0, vec_ppi_0);
        //nrf_uarte_subscribe_set(&nrf_uarte_reg, NRF_UARTE_TASK_STOPRX, *vec_ppi);
        err_code = nrfx_dppi_channel_assign(vec_ppi_0, (uint32_t)&NRF_GPIOTE->EVENTS_IN[2], nrf_uarte_task_address_get(&nrf_uarte_reg, NRF_UARTE_TASK_STARTTX));
        LP_DRV_ERR_CHECK(nrfx_err_to_zephyr_err(err_code), "Error assigning S1 rising edge event to STARTTX task");


        GET_DPPI_FROM_VEC(ppi_channels, 1, vec_ppi_1);
        //nrf_uarte_publish_set(&nrf_uarte_reg, NRF_UARTE_EVENT_RXSTARTED, *vec_ppi);
        err_code = nrfx_dppi_channel_assign(vec_ppi_1, nrf_uarte_event_address_get(&nrf_uarte_reg, NRF_UARTE_EVENT_ENDTX), (uint32_t)&NRF_GPIOTE->TASKS_CLR[0]);
        ...
        FREE_REALLOC_DPPI(ppi_channels, 2, vec_ppi_2);
        FREE_REALLOC_DPPI(ppi_channels, 3, vec_ppi_3);
...
static int nrf_lp_uarte_init(const struct lp_uarte_dev* dev)
{
    ...
    vector_init(&ppi_channels, DPPI_CHANNELS_REQUIRED, sizeof(uint8_t));
    /* Configuring NRF PPI peripheral to sync events to S0, S1, S2 GPIO events. */
    /* We will need at least four PPI channels I think */
    for(int i = 0; i < DPPI_CHANNELS_REQUIRED; i ++)
    {
        uint8_t c = 0;
        err_code = nrfx_dppi_channel_alloc(&c);
        if(err_code == NRFX_SUCCESS)
        {
            vector_push_back(&ppi_channels, &c);
        }
        else
        {
...
static int nrf_lp_uarte_deinit(const struct lp_uarte_dev* dev)
{
    ...
    for(int i = 0; i < DPPI_CHANNELS_REQUIRED; i ++)
    {
        GET_DPPI_FROM_VEC(ppi_channels, i, vec_ppi);
        nrfx_dppi_channel_free(vec_ppi);
    }
    vector_deinit(&ppi_channels);
    return 1;
}
